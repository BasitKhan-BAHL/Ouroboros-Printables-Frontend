...
export function meta() {
  return [
    { title: "Manage Subscriptions – Ouroboros Printables" },
    { name: "description", content: "Choose your license and subscription plan." },
  ];
}
  ...
  const plans = [
    {
      id: "monthly",
      title: "Monthly",
      desc: "Flexible monthly access",
      price: "£9.99",
      period: "/month",
      features: ["Access to all templates", "New releases included", "Cancel anytime", "Email support"],
    },
    {
      id: "yearly",
      title: "Yearly",
      ...
      period: "/year",
      badge: "Most Popular",
      features: ["Access to all templates", "New releases included", "Priority support", "Exclusive yearly bonuses", "2 months free"],
    },
    {
      id: "lifetime",
      title: "Lifetime",
      ...
      price: "£199",
      period: "one-time",
      features: ["Lifetime access to all templates", "All future updates included", "VIP support", "Early access to new products", "Commercial license included"],
    },
  ];

  const handleContinue = () => {
    updateSubscription({ license: selectedLicense, plan: selectedPlan });
    
    if (actionParam === "add_to_cart" && productIdParam) {
      // In a real flow, you might want to add to cart here if it wasn't added yet, or just go to cart.
      // But the cart state might not cross the redirect cleanly unless we add it here.
  ...
  return (
    <div className="mx-auto max-w-5xl px-6 py-16 sm:px-8">
      <div className="text-center">
        ...
        <p className="mt-2 font-secondary text-primary-600">Manage your licenses and subscriptions</p>
      </div>
      ...
      <div className="mt-16">
        <div className="flex items-center gap-2">
          <LightningIcon />
          <h2 className="font-primary text-2xl font-bold text-primary-900">Subscription Plans</h2>
        </div>
          ...
          {plans.map((plan) => {
            ...
            return (
              <div
                ...
                className={`relative flex cursor-pointer flex-col rounded-xl border p-6 transition ${
                  isSelected ? "border-2 border-primary-900" : "border-primary-200 hover:border-primary-400"
                } bg-white ${plan.badge ? "mt-4" : ""}`}
              >
                {plan.badge && (
                  <div className="absolute -top-3 left-1/2 flex -translate-x-1/2 items-center justify-center rounded-full bg-primary-900 px-3 py-1 text-xs font-semibold text-white">
                    {plan.badge}
                  </div>
                )}
                <div className="flex gap-3">
                  ...
                  <div>
                    <h3 className="font-primary text-lg font-bold text-primary-900">{plan.title}</h3>
                    <p className="font-secondary text-sm text-primary-600">{plan.desc}</p>
                  </div>
                </div>

                <div className="mt-6 flex items-baseline gap-1">
                  <span className="font-primary text-3xl font-bold text-primary-900">{plan.price}</span>
                  {plan.period && <span className="font-secondary text-sm text-primary-600">{plan.period}</span>}
                </div>
